...
The UART serial port data is available as a raw TCP stream. Note that there is no security checks at all,
...
Here is a usage example:
  ...
  begin
    ...
    if ( 0 != run_shell_command_dpi( shell_cmd_exit_code,
                                     "gnome-terminal --command \"sh -c 'socat READLINE TCP4:localhost:%d'\" &",
                                     `MY_TCP_PORT ) )
      begin
